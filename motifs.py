...
def find_gquadruplex(seq):
    ...
    return [dict(Class="Quadruplex", Subtype="Canonical_G-Quadruplex", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
...
def find_relaxed_gquadruplex(seq):
    ...
    return [dict(Class="Quadruplex", Subtype="Relaxed_G-Quadruplex", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="G4Hunter", Score=f"{g4hunter_score(m.group(0)):.2f}")
            for m in overlapping_finditer(pattern, seq)]

def find_bulged_gquadruplex(seq):
    pattern = r"(?=(G{3,}[ATGC]{0,3}G{3,}[ATGC]{0,3}G{3,}[ATGC]{0,3}G{3,}))"
    return [dict(Class="Quadruplex", Subtype="Bulged_G-Quadruplex", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="G4Hunter (bulge)", Score=f"{g4hunter_score(m.group(0)):.2f}")
            for m in overlapping_finditer(pattern, seq)]

def find_imotif(seq):
    pattern = r"(?=(C{3,}(?:[ATGC]{0,7}C{3,}){3}))"
    return [dict(Class="Quadruplex", Subtype="i-Motif", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
...
def find_gtriplex(seq):
    ...
    return [dict(Class="Triplex", Subtype="G-Triplex", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="G4Hunter", Score=f"{g4hunter_score(m.group(0)):.2f}")
            for m in overlapping_finditer(pattern, seq)]

def find_bipartite_gquadruplex(seq):
    pattern = r"(?=(G{3,}(?:[ATGC]{0,30}G{3,}){3}))"
    return [dict(Class="Quadruplex", Subtype="Bipartite_G-Quadruplex", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="G4Hunter", Score=f"{g4hunter_score(m.group(0)):.2f}")
            for m in overlapping_finditer(pattern, seq)]

def find_multimeric_gquadruplex(seq):
    pattern = r"(?=(G{3,}(?:[ATGC]{0,12}G{3,}){4,}))"
    return [dict(Class="Quadruplex", Subtype="Multimeric_G-Quadruplex", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="G4Hunter", Score=f"{g4hunter_score(m.group(0)):.2f}")
            for m in overlapping_finditer(pattern, seq)]

def find_zdna(seq):
    pattern = r"(?=((?:CG){6,}))"
    return [dict(Class="Z-DNA", Subtype="CG_Repeat", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="ZSeeker", Score=f"{zseeker_score(m.group(0)):.2f}")
            for m in overlapping_finditer(pattern, seq)]

def find_hdna(seq):
    pattern = r"(?=(T{3,}[ATGC]{1,7}A{3,}))"
    return [dict(Class="H-DNA", Subtype="T-A", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="None", Score="0")
            for m in overlapping_finditer(pattern, seq)]

def find_sticky_dna(seq):
    pattern = r"(?=(CTGCTGCTGCTG))"
    return [dict(Class="Sticky_DNA", Subtype="CTG", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="None", Score="0")
            for m in overlapping_finditer(pattern, seq)]

def find_slipped_dna(seq):
    pattern = r"(?=((?:AT){6,}))"
    return [dict(Class="Slipped_DNA", Subtype="AT_Slippage", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="None", Score="0")
            for m in overlapping_finditer(pattern, seq)]

def find_cruciform(seq):
    pattern = r"(?=(A{4,}TTTT))"
    return [dict(Class="Cruciform", Subtype="A-T", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="None", Score="0")
            for m in overlapping_finditer(pattern, seq)]

def find_bent_dna(seq):
    pattern = r"(?=(A{6,7}|T{6,7}))"
    return [dict(Class="Bent_DNA", Subtype="Poly-A/T", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="None", Score="0")
            for m in overlapping_finditer(pattern, seq)]

def find_apr(seq):
    pattern = r"(?=(?:AAATT){2,})"
    return [dict(Class="A-Phased_Repeat", Subtype="APR", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="None", Score="0")
            for m in overlapping_finditer(pattern, seq)]

def find_mirror_repeat(seq):
    pattern = r"(?=(ATCGCGAT))"
    return [dict(Class="Mirror_Repeat", Subtype="ATCGCGAT", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="None", Score="0")
            for m in overlapping_finditer(pattern, seq)]
...
def find_polyG(seq):
    pattern = r"(?=(G{6,}))"
    return [dict(Class="Direct_Repeat", Subtype="Poly-G", Start=m.start()+1, End=m.start()+len(m.group(0)), Length=len(m.group(0)),
                 Sequence=wrap(m.group(0)), ScoreMethod="None", Score="0")
            for m in overlapping_finditer(pattern, seq)]
